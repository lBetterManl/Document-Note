...
2017年08月02日 23:29:32

标签： [maven](http://so.csdn.net/so/search/s.do?q=maven&t=blog)[pom-xml](http://so.csdn.net/so/search/s.do?q=pom-xml&t=blog)[pom配置](http://so.csdn.net/so/search/s.do?q=pom%E9%85%8D%E7%BD%AE&t=blog)[groupid](http://so.csdn.net/so/search/s.do?q=groupid&t=blog)

个人分类： [Maven](https://blog.csdn.net/lmb55/article/category/7478736)

版权声明：本文为博主原创文章，无需授权即可转载，甚至无需保留以上版权声明，转载时请务必注明作者。	https://blog.csdn.net/lmb55/article/details/76599945

就像web项目的核心是web.xml一样，Maven项目的核心是pom.xml，POM（project object model，项目对象模型）定义了项目的基本信息，用于描述项目如何构建，如何声明依赖等等。如下所示，介绍了pom文件中各个标签的用法和含义：
